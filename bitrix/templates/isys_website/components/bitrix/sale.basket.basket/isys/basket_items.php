...
<h3>
    В вашей корзине <?=$Quantity?> товаров на сумму
    <span class="darkbluetext" id="allSum_FORMATED"><?=str_replace(" ", "&nbsp;", $arResult["allSum_FORMATED"])?></span>.
    Суммарный вес
    <span  class="darkbluetext" id="allWeight_FORMATED"><?=$arResult["allWeight_FORMATED"]?></span>.
</h3><br>
...
<?foreach ($arResult["GRID"]["ROWS"] as $id=>$arItem):
    if ($arItem["DELAY"] == "N" && $arItem["CAN_BUY"] == "Y"):?>
        ...
        <div class="col-xs-2">
            ...
            <div class="input-group number-spinner-vert btn-block">
                <span class="input-group-btn btn-block">
                    <a href="javascript:void(0);" class="btn btn-sm  btn-block btn-primary btnplus plus" data-dir="up"
                        onclick="setQuantity(<?=$arItem["ID"]?>, 1, 'up', <?=$useFloatQuantityJS?>);">
                        <span class="glyphicon glyphicon-plus"></span></a>
                </span>
                <input type="text" class="prodquant form-control text-center btn-block"
                       id="QUANTITY_INPUT_<?=$arItem["ID"]?>" name="QUANTITY_INPUT_<?=$arItem["ID"]?>"
                       maxlength="18" min="0" <?=$max?> step="<?=$ratio?>"
                       onchange="updateQuantity('QUANTITY_INPUT_<?=$arItem["ID"]?>', '<?=$arItem["ID"]?>', <?=$ratio?>, <?=$useFloatQuantityJS?>)"
                       value="<?=$arItem["QUANTITY"]?>">
                <span class="input-group-btn btn-block">
                    <a href="javascript:void(0);" class="btn btn-sm  btn-block btn-info btnminus minus" data-dir="dwn"
                        onclick="setQuantity(<?=$arItem["ID"]?>, 1, 'down', <?=$useFloatQuantityJS?>);">
                        <span class="glyphicon glyphicon-minus"></span></a>
                </span>
                <input type="hidden" id="<?=$arItem['ID']?>_pricer" name="<?=$arItem['ID']?>_pricer" value="<?=$arItem["PRICE_FORMATED"]?>" />
        ...
        <div class="col-xs-2 nopad text-right">
            ...
            <a href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delay"])?>" class="btn btn-primary btn-warning  margn-bot-10">
                <i class="fa fa-minus-circle" aria-hidden="true"></i><span class="hidden-xs"> Отложить</span>
            </a>
...
<h3>
    В вашей корзине пока нет товаров. <a href="/index.php" class="darkgreentext">Перейти к покупкам.</a>
</h3><br>
...
<script type="text/javascript">
    $(".number-spinner-vert .btn-sm").click(function() {
        var inp = $(this).closest(".input-group").children("input.prodquant"),
            inpId = $(inp).attr('id');
        var id = String(inpId).replace("QUANTITY_INPUT_","");

        var price  = parseFloat($("#"+id+"_pricer").val());
        var curCnt = parseInt($("span.baskettotalquantity").text());
        var curSum = parseFloat($("span.baskettotalprice").text());
        var count, sum;
        ...
        $("span.baskettotalquantity").text(count);
        $("span.baskettotalprice").text(sum+" руб.");
    });
</script>
